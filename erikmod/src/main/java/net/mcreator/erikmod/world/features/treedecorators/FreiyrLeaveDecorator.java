...
public class FreiyrLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<FreiyrLeaveDecorator> CODEC = MapCodec.unit(FreiyrLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("erikmod:freiyr_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public FreiyrLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.WARPED_STEM.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.WARPED_STEM.defaultBlockState(), direction));
