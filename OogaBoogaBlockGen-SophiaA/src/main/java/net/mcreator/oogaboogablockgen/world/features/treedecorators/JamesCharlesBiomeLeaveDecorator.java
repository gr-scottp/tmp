...
public class JamesCharlesBiomeLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<JamesCharlesBiomeLeaveDecorator> CODEC = MapCodec.unit(JamesCharlesBiomeLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("oogaboogablockgen:james_charles_biome_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public JamesCharlesBiomeLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.CAVE_VINES.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.CAVE_VINES.defaultBlockState(), direction));
