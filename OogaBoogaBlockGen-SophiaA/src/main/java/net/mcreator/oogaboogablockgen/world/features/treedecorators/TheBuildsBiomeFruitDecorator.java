...
public class TheBuildsBiomeFruitDecorator extends CocoaDecorator {
	public static MapCodec<TheBuildsBiomeFruitDecorator> CODEC = MapCodec.unit(TheBuildsBiomeFruitDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("oogaboogablockgen:the_builds_biome_tree_fruit_decorator"), () -> DECORATOR_TYPE);
	}

	public TheBuildsBiomeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(Blocks.LIGHT_BLUE_CONCRETE.defaultBlockState(), direction1));
						}
					}
				}
			});
