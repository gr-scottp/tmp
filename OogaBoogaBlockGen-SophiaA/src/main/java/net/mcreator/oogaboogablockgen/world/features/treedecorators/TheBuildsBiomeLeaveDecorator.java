...
public class TheBuildsBiomeLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<TheBuildsBiomeLeaveDecorator> CODEC = MapCodec.unit(TheBuildsBiomeLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("oogaboogablockgen:the_builds_biome_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public TheBuildsBiomeLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.WARPED_ROOTS.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.WARPED_ROOTS.defaultBlockState(), direction));
